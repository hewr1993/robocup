...
int ball_filter(struct reg_t* g, struct cmv_data * data)
{
     if (g->color != COLOR_B + 1){
		if(g->color != COLOR_S + 1&&g->color != COLOR_G + 1)
			return 1;
		if (g->size < 200)
			return 0;
		return 1;
...
//****************************By 彭瑾玥******************************
	...
	int x1start = (1-sqrt(2)/2)*(g->x2-g->x1) + g->x1;
	int y1start = (1-sqrt(2)/2)*(g->y2-g->y1) + g->y1;
	int x2start = g->x2-(1-sqrt(2)/2)*(g->x2-g->x1);
	int y2start = g->y2-(1-sqrt(2)/2)*(g->y2-g->y1);
	

	for (ii=x1start; ii<x2start; ii ++)
 	{
		for(jj=y1start;jj<y2start;jj++)
		{
		...
		red_ratio = red_count* 1.0 / size0;
	//XXX为红色率的下限，需要调参数
	
	//if (red_ratio<0.65 && red_ratio > 3.1415926/4+0.1)
	if(red_ratio<0.9)
		return 0;
...
//BY PJY：对角红色率过滤
	...
	if(data->map[g->x1*(W_2+1)/2+g->y1/2]==COLOR_B)
		cor_red++;
	if(data->map[g->x1*(W_2+1)/2+g->y2/2]==COLOR_B)
		cor_red++;
	if(data->map[g->x2*(W_2+1)/2+g->y1/2]==COLOR_B)
		cor_red++;
	if(data->map[g->x2*(W_2+1)/2+g->y2/2]==COLOR_B)
		cor_red++;		
	if(cor_red>3)
		return 0;*/
	...
	//H=23CM
	//top: 
         int xtop=(g->x1+g->x2)/2,ytop=g->y1;
	//low: 
	 int xlow=(g->x1+g->x2)/2,ylow=g->y2;
	
	int rx_top,ry_top;
	int rx_low,ry_low;
	
        if (ytop<=H) {
            getxy(xtop,ytop,&rx_top,&ry_top,local_info[0].vparam);
        } else {
            getxy(xtop,ytop,&rx_top,&ry_top,local_info[1].vparam);
        }

	if (ylow<=H) {
            getxy(xlow,ylow,&rx_low,&ry_low,local_info[0].vparam);
        } else {
            getxy(xlow,ylow,&rx_low,&ry_low,local_info[1].vparam);
        }

	double r=0;

	double dc = sqrt( pow(rx_top-rx_low,2) + pow(ry_top-ry_low,2) );
	double bc = sqrt(rx_top*rx_top + ry_top*ry_top);
	double HH = 24.0;
	double fd = dc*1.0/bc*HH;
	double ac = sqrt(bc*bc + HH*HH);	
	r = fd *1.0/(1+ac/bc);
		
	//r = 23*1.0/(sqrt(rx_top*rx_top+ry_top*ry_top)*1.0/(sqrt(rx_top*rx_top+ry_top*ry_top)-sqrt(rx_low*rx_low+ry_low*ry_low)));	
...
//*********************************by pjy***************************	
	...
	int px1 = g->x1,py1 = (g->y1+g->y2)/2;
	int px2 = g->x2,py2 = py1;

	int rx1,rx2,ry1,ry2;

	if (py1<=H) {
            getxy(px1,py1,&rx1,&ry1,local_info[0].vparam);
	    getxy(px2,py2,&rx2,&ry2,local_info[0].vparam);
        } else {
            getxy(px1,py1,&rx1,&ry1,local_info[1].vparam);
	    getxy(px2,py2,&rx2,&ry2,local_info[1].vparam);
        }	
	double rdx = (rx1+rx2)/2,rdy = (ry1+ry2)/2;
	double b1d1 = sqrt(pow(rdx-rx1,2)+pow(rdy-ry1,2));
	double a1b1 = sqrt(rx1*rx1+ry1*ry1);
	double a1e1 = sqrt(rx_low*rx_low + ry_low*ry_low);
	double r1 = b1d1*1.0 / a1b1 * a1e1;
        if(r1>9)
		return 0;
...
int goal_detection(struct cmv_data *data, unsigned char* src,
                   struct spot * left, struct spot * right, int* isLeftArrived, int* isRightArrived, struct local_parse_info * local_tmp,struct local_parse_info **data_t,int color_tmp)
{
//    fprintf (stderr, "[g1]%d", *__CMV__);
    left->x = 0;
    right->x = 0;
    left->y = 0;
    right->y = 0;
    if (data->clist[color_tmp] == 0)
        return 0;
    ...
    *isRightArrived = 1;
    ...
    int maxYValue[W+10];
    int tmpMaxYValue[W+10];
    int rawYValue[W+10];

    //record maximum of maxYValue[] and its cordinary x value.
    int maxmaxY=0, maxmaxXofY=0;

    memset(maxYValue,0,sizeof(int) *W);
 
//fprintf (stderr, "[g2]%d", *__CMV__);
    int leftXEdge=2*data->cmax[color_tmp]->x1+1,rightXEdge=2*data->cmax[color_tmp]->x2-1;
    int maxYEdge=2*(data->cmax[color_tmp]->y2);
    ...
    for (; i<=rightXEdge&&i<W; i+=2) {
        ...
        for (; j>=0; j-=2) {
            ...
            if ( (data->map[pos] == (color_tmp + 1))
                    &&  (data->map[upos] == (color_tmp + 1)) ) {

		mylogfd(logfd,"map:%d,pos:%d\n",data->map[pos],pos);
                maxYValue[i]=j;
                maxYValue[i+1]=j;
                break;
            }
...
//fprintf (stderr, "[g3]%d", *__CMV__);
//    fprintf(stderr,"find max over:%x\n",local_tmp->vparam);
    memcpy(tmpMaxYValue,maxYValue,sizeof(int)*W);
    ...
    memcpy(tmpMaxYValue,maxYValue,sizeof(int)*W);
//fprintf (stderr, "[g4]%d", *__CMV__);
    memcpy(rawYValue,maxYValue,sizeof(int)*W);

//fprintf (stderr, "[g5]%d", *__CMV__);
    //calculate f'(x)
    for (i=rightXEdge; i>=0; i--) {
        ...
        if (i>=W-2)
            ...
            maxYValue[i]=(tmpMaxYValue[i+2]+tmpMaxYValue[i+1]-tmpMaxYValue[i-1]-tmpMaxYValue[i-2])/2;
    }
//fprintf (stderr, "[g6]%d", *__CMV__);
    memcpy(tmpMaxYValue,maxYValue,sizeof(int)*W);
    //calculate f''(x)
//    fprintf (stderr, "[g7]%d", *__CMV__);
    for (i=rightXEdge; i>=0; i--) {
        if (i>=W-2)
            maxYValue[i]=tmpMaxYValue[i]-tmpMaxYValue[i-2];
        else if (i<=1)
...
//fprintf (stderr, "[g8]%d", *__CMV__);
    ...
    for (i=rightXEdge; i>0; i--) {
        if (maxYValue[i]<=-1) {
            ...
            if (  (rightavg>leftavg)
                    &&(abs2(i-2*data->cmax[color_tmp]->x1)<35)) {
                ...
                *isLeftArrived = 0;
                if (maxYValue[i]<minfpp)
                    minfpp=maxYValue[i];
                         ...
                         && (abs2(i-2*data->cmax[color_tmp]->x2)<35)  ) {
                //if zero point meets the following conditions:
...
//fprintf (stderr, "[ga]%d", *__CMV__);
    if (*isRightArrived) {
        right->x=2*data->cmax[color_tmp]->x2-3;
        int max=-1;
...
{
    door_info *door_info_temp;
    int color_tmp;
    int flag=0;
    if (is_mine_door) {
        door_info_temp=&(local_tmp->mine_door);
        color_tmp=COLOR_S;
    } else {
        door_info_temp=&(local_tmp->enemy_door);
        color_tmp=COLOR_G;
    }
    //TODO 门的下边沿检测，稳定
    int tldx, tldy, trdx, trdy;
    door_info_temp->is_door_seen=1;
    door_info_temp->is_door_right=0;
    door_info_temp->is_door_left=0;
//    fprintf(stderr, "[a]");
//fprintf(stderr, "[D]%x\n",(int)door_info_temp );
    struct reg_t *reg = data_tmp->cmax[color_tmp];
    if (reg && data_tmp->clist[color_tmp]) {
        flag=1;
//fprintf(stderr, "[b]");
        struct spot p_left, p_right;
        int leftmost, rightmost;
//fprintf(stderr, "[D]%x\n",(int)door_info_temp );
door_info *__temp = door_info_temp;
int int_tmp = (int)door_info_temp;
//fprintf(stderr, "[CCC]%x, %x\n",(int)__temp, int_tmp );
__CMV__ = (int *)(&door_info_temp);
        goal_detection(data_tmp, yuv_buf, &p_left, &p_right, &leftmost, &rightmost,local_tmp,&local_tmp,color_tmp);
//        fprintf(stderr, "[c]");
//fprintf(stderr, "[DD]%x\n",(int)door_info_temp );
//fprintf(stderr, "[DDD]%x, %x\n",(int)__temp, int_tmp );
        //fprintf(stderr,"&&in find door:%d %d %d\n",data_tmp,door_info_temp,yuv_buf);
        if (reg->x1 < 4)
            leftmost = 1;
        if (reg->x2 > 157)
            rightmost = 1;
...
#define DOUBLE_MIN 0.000001
        if(r < DOUBLE_MIN && r > -DOUBLE_MIN){
            flag=1;
        }else{
//            fprintf(stderr, "%d, %d, %f\n", leftmost,rightmost, r);
            if (!leftmost) {
//fprintf(stderr, "[l]\n");
//fprintf(stderr, "[D]%x\n",(int)door_info_temp );
                door_info_temp->door_acc_pos.x = lx + DOOR_LENGTH_2 * (rx - lx) / r;
//fprintf(stderr, "[L]%d\n", door_info_temp->door_acc_pos.x);
                door_info_temp->door_acc_pos.y = ly + DOOR_LENGTH_2 * (ry - ly) / r;
//fprintf(stderr, "[L]\n");
            } else if (!rightmost) {
                door_info_temp->door_acc_pos.x = rx - DOOR_LENGTH_2 * (rx - lx) / r;
                door_info_temp->door_acc_pos.y = ry - DOOR_LENGTH_2 * (ry - ly) / r;
            } else {
                getxy(160, (reg->y2)<<1, &(door_info_temp->door_acc_pos.x), &(door_info_temp->door_acc_pos.y), local_tmp->vparam);
//                fprintf(stderr,"$$$$$$$DOOR: %d %d %d %d \n",160, (reg->y2)<<1, door_info_temp->door_acc_pos.x, door_info_temp->door_acc_pos.y);
            }
//fprintf(stderr, "[e]");
            if (!leftmost) {//看到左门柱
                door_info_temp->is_door_left = 1;
                door_info_temp->door_left.x = lx + DOOR_OFFSET * (rx - lx) / r;
                door_info_temp->door_left.y = ly + DOOR_OFFSET * (ry - ly) / r;
            }
            if (!rightmost) {//看到右门柱
                door_info_temp->is_door_right = 1;
                door_info_temp->door_right.x = rx - DOOR_OFFSET * (rx - lx) / r;
                door_info_temp->door_right.y = ry - DOOR_OFFSET * (ry - ly) / r;
            }
//fprintf(stderr, "[f]");
            if (door_info_temp->door_acc_pos.y > DISTANCE_FAR) {
                door_info_temp->door_pos.y = LOC_Y_FAR;
//fprintf(stderr, "[g]");
            } else {
                door_info_temp->door_pos.y = DISTANCE_NEAR;
            }
            if (door_info_temp->door_acc_pos.x <-20)
                door_info_temp->door_pos.x = LOC_LEFT;
            else if (door_info_temp->door_acc_pos.x > 20)
                door_info_temp->door_pos.x = LOC_RIGHT;
            else
                door_info_temp->door_pos.x = 0;
        }
    } 
//fprintf(stderr, "[h]");
    if(!flag){
        /*door INVALIED*/
        door_info_temp->is_door_seen=0;
        door_info_temp->is_door_left=0;
        door_info_temp->is_door_right=0;
    }
//fprintf(stderr, "[i]");
...
void translate_init(struct local_parse_info * local_info)
{
    ...
    for (i=0; i<2; i++) {
        ...
        local_info[i].enemy_door.is_door_seen=0;
        local_info[i].enemy_door.is_door_right=0;
        local_info[i].enemy_door.is_door_left=0;

        local_info[i].mine_door.is_door_seen=0;
        local_info[i].mine_door.is_door_right=0;
        local_info[i].mine_door.is_door_left=0;
