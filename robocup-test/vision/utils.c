...
//static void yuv422to420p(unsigned char* src, unsigned char* des, int width, int height);
...
//added by jascon
void
yuv422to420p(unsigned char* src, unsigned char* des, int width, int height)
{
    int i=0, j=0;
    unsigned char* pc0=src;
    unsigned char* pc1=src+2*width;
    unsigned char* y1=des;
    unsigned char* y2=des+width;
    unsigned char* u=des+width*height;
    unsigned char* v=u+width*height/4;
    for (i=0; i<height; i+=2) {
        for (j=0; j<width; j+=2) {
            //fill the cell for yuv420p's y value
            *y1=pc0[0];
            *(y1+1)=pc0[2];
            *y2=pc1[0];
            *(y2+1)=pc1[2];
